...
interface AtariGoBaselineDigest {
  generatedAt: string;
  games: number;
  decisions: number;
  t1: {
...
interface HardeningSummary {
  ...
  atariGoBaseline: AtariGoBaselineDigest | null;
}
...
async function readAtariGoBaselineDigest(): Promise<AtariGoBaselineDigest | null> {
  const baselinePath = join(import.meta.dir, '..', 'artifacts', 'atari-go-baseline', 'latest', 'baseline.json');
  try {
    const raw = await readFile(baselinePath, 'utf8');
    const data = JSON.parse(raw) as {
      generatedAt: string;
    ...
    const failedSteps = data.nC3.monotonicSteps
      .filter((step) => !step.pass)
      .map((step) => `N${step.toLevel}>=N${step.fromLevel}`);
    ...
    return {
      ...
      t2Monotonic: {
        failedSteps,
        passCount: data.nC3.monotonicPassCount,
        passRequired: data.nC3.monotonicPassRequired,
        passAll: data.nC3.passAll,
      },
    };
  } catch {
    return null;
...
async function writeSummary(summary: HardeningSummary): Promise<void> {
  ...
  const outputDir = join(import.meta.dir, '..', 'artifacts', 'hardening', 'f3.2', stamp);
  const latestDir = join(import.meta.dir, '..', 'artifacts', 'hardening', 'f3.2', 'latest');
...
async function main(): Promise<void> {
  ...
  const atariGoBaseline = await readAtariGoBaselineDigest();
...
main().catch((error) => {
  console.error('[hardening-f3.2] fatal', error);
  process.exit(1);
});
