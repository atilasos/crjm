...
def _make_batch(data: dict[str, np.ndarray], idx: np.ndarray,
    ...
    """Constrói um batch (features, pi, legal_mask, z) com augmentação D4."""
    feats = np.stack([
        encode_features(data["boards"][i], int(data["to_play"][i]), int(data["last_move"][i]))
        for i in idx
    ])
    x = torch.from_numpy(feats)
    pi = torch.from_numpy(data["pi"][idx]).view(-1, SIZE, SIZE)
    mask = torch.from_numpy(data["legal_mask"][idx]).view(-1, SIZE, SIZE)
    z = torch.from_numpy(data["z"][idx])
    syms = rng.integers(0, 8, idx.size)
    for k in range(1, 8):
        sel = torch.from_numpy(syms == k)
        if sel.any():
            x[sel] = _d4(x[sel], k)
            pi[sel] = _d4(pi[sel], k)
            mask[sel] = _d4(mask[sel], k)
    return x, pi.reshape(-1, rules.NUM_CELLS), mask.reshape(-1, rules.NUM_CELLS), z
