...
def _make_child(node: Node, edge: int) -> Node:
    ...
    if status == rules.WIN:
        # quem jogou capturou e venceu: valor -1 para o jogador a mover no filho
        child = Node(new_board, opp, move, terminal=-1.0)
    elif status == rules.DRAW:
        child = Node(new_board, opp, move, terminal=0.0)
    else:
...
def mcts_decide(
    ...
    """Corre `sims` simulações; devolve (visits (81,) int32, legais da raiz).
    ...
    visits = np.zeros(rules.NUM_CELLS, dtype=np.int32)
