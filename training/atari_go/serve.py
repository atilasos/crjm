...
DEFAULT_MODEL = "runs/az-v1/best.pt"
FALLBACK_MODEL = "runs/smoke/best.pt"
...
class MoveRequest(BaseModel):
    ...
    toPlay: int
    lastMove: int | None = Field(default=None, ge=0, le=rules.NUM_CELLS - 1)
    timeBudgetMs: int | None = None
    seed: int | None = None


def _tactical_scan(board: np.ndarray, to_play: int) -> tuple[list[int], list[int], list[int]]:
    """Uma passagem pelas jogadas legais.

    Devolve (capturas imediatas, defesas seguras, jogadas inseguras).
    Insegura = após a jogada, algum grupo próprio fica com 1 liberdade
    (o adversário captura — e vence — na resposta).
    """
    ...
    for mv in legal:
        mv = int(mv)
        new_board, status = rules.play(board, to_play, mv)
        if status == rules.WIN:
            wins.append(mv)
            continue
        if status == rules.DRAW:
            safe.append(mv)  # adversário sem jogadas legais
            continue
        libs = rules.stone_liberty_counts(new_board)
        own_in_atari = bool(np.any((new_board == to_play) & (libs == 1)))
        (unsafe if own_in_atari else safe).append(mv)
    return wins, safe, unsafe
...
def _compute_move(req: MoveRequest) -> dict:
    ...
    if req.toPlay not in (rules.BLACK, rules.WHITE):
        raise HTTPException(status_code=422, detail="toPlay deve ser 1 (pretas) ou 2 (brancas)")
    if any(cell not in (0, 1, 2) for cell in req.board):
        raise HTTPException(status_code=422, detail="board deve conter apenas 0, 1 ou 2")
    ...
    with _serve_lock:
        ...
        wins, safe, unsafe = _tactical_scan(board, to_play)
        if not wins and not safe and not unsafe:
        ...
        if wins:
            ...
            return {
                "move": wins[0],
                "value": 1.0,
                "topMoves": [{"move": wins[0], "prior": 1.0, "visits": 0}],
                "stats": {
                    "sims": 0,
                    "elapsedMs": round(elapsed_ms, 2),
                    "engine": "server-nn",
                    "tactical": "immediate-capture",
                },
            }

        restrict = set(safe) if safe and unsafe else None
    ...
    return {
        ...
        "stats": {
            ...
            "elapsedMs": round(elapsed_ms, 2),
            "engine": "server-nn",
            "tactical": "defense-restricted" if restrict is not None else None,
        },
    }
