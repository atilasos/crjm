    ...
    <section className="rounded-xl border px-4 py-3 [background:var(--painel)] [border-color:var(--linha)]">
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        {moves.slice(0, 3).map((candidate) => (
          ...
          >
            <p className="font-semibold [color:var(--tinta)]">{t("Opção #")}{t(candidate.rank)}</p>
            <p className="mt-1 text-[11px] [color:var(--tinta-suave)]">{t("Segmento: ")}{t(formatMove(candidate.move))}
            </p>
