...
import { gerarPosicoesValidas, posToKey, keyToPos, LADO_TABULEIRO } from './types';
...
export function criarTabuleiroInicial(): Record<string, Celula> {
  const tabuleiro: Record<string, Celula> = {};
  const posicoes = gerarPosicoesValidas();

  for (const pos of posicoes) {
    tabuleiro[posToKey(pos)] = 'vazia';
  }
...
export function encontrarGrupos(tabuleiro: Record<string, Celula>, cor: 'preta' | 'branca'): Grupo[] {
  const grupos: Grupo[] = [];
  const visitadas = new Set<string>();

  for (const [key, celula] of Object.entries(tabuleiro)) {
    if (celula === cor && !visitadas.has(key)) {
      ...
      while (fila.length > 0) {
        ...
        const atualCelula = tabuleiro[atualKey];
        ...
        for (const vizinho of getVizinhos(pos)) {
          if (dentroDoTabuleiro(vizinho)) {
            ...
            if (!visitadas.has(vizinhoKey) && tabuleiro[vizinhoKey] === cor) {
...
export function calcularPontuacao(tabuleiro: Record<string, Celula>, cor: 'preta' | 'branca'): Pontuacao {
  const grupos = encontrarGrupos(tabuleiro, cor);

  // Contar total de peças
  let totalPecas = 0;
  for (const [, celula] of Object.entries(tabuleiro)) {
...
// Obter casas vazias
export function getCasasVazias(tabuleiro: Record<string, Celula>): Posicao[] {
...
export function colocarPeca(
  ...
  if (state.tabuleiro[key] !== 'vazia') return state;

  const novoTabuleiro = { ...state.tabuleiro };
  novoTabuleiro[key] = cor;
...
function determinarVencedor(tabuleiro: Record<string, Celula>): GameStatus {
...
export function cancelarJogadaEmCurso(state: ProdutoState): ProdutoState {
  ...
  const novoTabuleiro = { ...state.tabuleiro };
  novoTabuleiro[posToKey(state.jogadaEmCurso.pos1)] = 'vazia';
...
export function jogadaComputador(state: ProdutoState): ProdutoState {
  ...
  if (state.primeiraJogada) {
    ...
    if (state.tabuleiro[posToKey(centro)] === 'vazia') {
    ...
    for (const cor1 of cores) {
      for (const pos2 of casasVazias) {
        ...
        for (const cor2 of cores) {
          // Simular jogada
          const tabTemp = { ...state.tabuleiro };
          tabTemp[posToKey(pos1)] = cor1;
          tabTemp[posToKey(pos2)] = cor2;
