  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 font-semibold">
        <span className="inline-flex h-6 w-6 items-center justify-center rounded-full text-xs font-bold text-white [background:var(--jogo-atari)]">{t("IA")}</span>{t("Dica do turno")}<span className="rounded-full px-2 py-0.5 text-xs font-medium [background:color-mix(in_srgb,var(--jogo-atari)_15%,var(--painel))] [color:var(--tinta-suave)]">
          {t(hintLevel)}
        </span>
      </div>
      {isLoading ? (
