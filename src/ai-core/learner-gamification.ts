...
export function hydrateProfileFromSnapshot(input: Pick<GamificationProfile, 'totalXp' | 'streakDays' | 'lastActiveDate' | 'gameProgress' | 'recentEvents'>): GamificationProfile {
...
export function deriveAchievementUnlocks(input: Pick<GamificationProfile, 'gameProgress' | 'recentEvents' | 'lastActiveDate'>): Record<string, AchievementUnlock> {
  ...
  if (input.gameProgress['atari-go'].wins >= 1) maybeUnlock('atari_hunter');
  if (input.gameProgress.produto.played >= 1) maybeUnlock('balanced_builder');
  if (input.gameProgress.nex.played >= 1) maybeUnlock('bridge_builder');
